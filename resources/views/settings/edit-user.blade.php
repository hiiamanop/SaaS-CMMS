    ...
    <form action="{{ route('settings.users.update', $user) }}" method="POST" class="bg-white rounded-lg border border-gray-200 p-6 space-y-5">
        ...
        <div>
            ...
            <select name="role" required class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-gray-900">
                ...
                <option value="supervisor" {{ old('role', $user->role) === 'supervisor' ? 'selected' : '' }}>Supervisor (SPV ONM)</option>
                <option value="technician" {{ old('role', $user->role) === 'technician' ? 'selected' : '' }}>Teknisi ONM</option>
                <option value="pm" {{ old('role', $user->role) === 'pm' ? 'selected' : '' }}>Project Manager (PM)</option>
            </select>
        </div>
        <div>
