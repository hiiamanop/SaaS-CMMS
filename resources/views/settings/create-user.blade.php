...
<div class="max-w-lg mx-auto space-y-6">
    ...
    <form action="{{ route('settings.users.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg border border-gray-200 p-6 space-y-5">
        ...
        <div x-data="{ show: false }">
            ...
            <div class="relative">
                <input x-bind:type="show ? 'text' : 'password'" name="password" required class="w-full rounded-md border border-gray-300 pl-3 pr-10 py-2 text-sm focus:ring-2 focus:ring-gray-900">
        ...
        <div x-data="{ show: false }">
            ...
            <div class="relative">
                <input x-bind:type="show ? 'text' : 'password'" name="password_confirmation" required class="w-full rounded-md border border-gray-300 pl-3 pr-10 py-2 text-sm focus:ring-2 focus:ring-gray-900">
                <button type="button" @click="show = !show" class="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-gray-600 focus:outline-none">
                    <svg x-show="!show" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
