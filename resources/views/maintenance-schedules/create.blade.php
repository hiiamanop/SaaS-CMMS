...
@section('breadcrumb')
    ...
    <span class="text-gray-400">/</span>
    <span class="text-gray-700 font-medium">Jadwal Baru</span>
@endsection
@section('content')
@php
...
<div class="max-w-none">
    ...
    <form action="{{ route('maintenance-schedules.store') }}" method="POST" class="space-y-6"
          x-data="{
              shutdown: {{ old('shutdown_required') ? 'true' : 'false' }},
              frequency: '{{ old('frequency', 'monthly') }}',
              locationId: '{{ old('location_id', $userLocation?->id ?? '') }}',
              trafoName: '{{ old('trafo_name', '') }}',
              trafoOptions: [],
              async loadTrafos() {
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="frequency" required x-model="frequency"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand @error('frequency') border-red-400 @enderror">
            ...
            <div class="overflow-x-auto" x-show="frequency === 'monthly'">
                <table class="text-xs border-collapse w-full">
                    ...
                    <tbody>
                        <tr>
                            @foreach(range(1,12) as $month)
                            @php $key = $month.'_1'; $checked = isset($oldWeeks[$key]); @endphp
                ...
                <table class="text-xs border-collapse w-full max-w-md">
                    ...
                    <tbody>
                        <tr>
                            ...
                            @php $key = $q.'_1'; $checked = isset($oldWeeks[$key]); @endphp
                ...
                <table class="text-xs border-collapse w-full max-w-xs">
                    ...
                    <tbody>
                        <tr>
                            ...
                            @php $key = $s.'_1'; $checked = isset($oldWeeks[$key]); @endphp
                            <td class="border border-gray-300 px-4 py-4 text-center">
                                <input type="checkbox" name="planned_weeks[{{ $key }}]" value="1"
                                       {{ $checked ? 'checked' : '' }}
                                       class="w-5 h-5 rounded border-gray-300 text-brand focus:ring-brand cursor-pointer">
        ...
        <div class="flex gap-3">
            <button type="submit" class="px-6 py-2.5 bg-brand-dark text-white font-bold rounded-lg text-sm font-bold hover:bg-opacity-90 shadow-md transition-all">
                Simpan Jadwal
            </button>
            <a href="{{ route('maintenance-schedules.index') }}" class="px-6 py-2.5 border border-gray-300 text-gray-700 rounded-lg text-sm font-medium hover:bg-opacity-90 transition-all">
                Batal
            </a>
        </div>
    </form>
