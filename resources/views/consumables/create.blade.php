    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <form action="{{ route('consumables.store') }}" method="POST" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div><label class="block text-sm font-medium text-gray-700 mb-1.5">{!! field_label('consumables', 'category', 'Category') !!}</label><input name="category" value="{{ old('category') }}" {!! field_attributes('consumables', 'category') !!} class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand"></div>
                @endif

                @if(!field_is_hidden('consumables', 'unit'))
                <div><label class="block text-sm font-medium text-gray-700 mb-1.5">{!! field_label('consumables', 'unit', 'Unit') !!}</label><input name="unit" value="{{ old('unit','pcs') }}" {!! field_attributes('consumables', 'unit') !!} class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand"></div>
                @endif
