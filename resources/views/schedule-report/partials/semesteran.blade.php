...
$semesterSchedules = \App\Models\MaintenanceSchedule::where('frequency', 'quarterly')
...
@php
    $items = (array)($semSchedule->item_pekerjaan ?? []);
...
<div class="mb-4 bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-sm text-yellow-800 flex items-center gap-2">
    ...
    <span><strong>{{ $semSchedule->equipment_name }}</strong> — belum ada item pekerjaan. <a href="{{ route('maintenance-schedules.show', $semSchedule) }}" class="underline font-medium">Tambahkan item di sini.</a></span>
</div>
@php continue; @endphp
@endif
<div class="mb-4">
<div class="px-4 py-2 bg-blue-50 border-b border-blue-100 text-sm font-semibold text-blue-800">
    {{ $semSchedule->title ?: $semSchedule->equipment_name }}
</div>
<div class="space-y-4">
@foreach($semestersList as $semNum => $semInfo)
