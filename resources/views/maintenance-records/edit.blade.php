    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <form action="{{ route('maintenance-records.update',$maintenanceRecord) }}" method="POST" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div><label class="block text-sm font-medium text-gray-700 mb-1.5">Downtime (minutes)</label><input name="downtime_minutes" type="number" min="0" value="{{ old('downtime_minutes',$maintenanceRecord->downtime_minutes) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"></div>
            </div>
