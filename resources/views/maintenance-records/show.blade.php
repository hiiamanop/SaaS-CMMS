...
@section('breadcrumb')<span class="text-gray-400">/</span><a href="{{ route('work-orders.index', ['tab' => 'records']) }}" class="hover:text-gray-800">Records</a><span class="text-gray-400">/</span><span class="text-gray-700 font-medium">{{ $maintenanceRecord->record_number }}</span>@endsection
@section('content')
...
<div class="space-y-5">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500">
                    {{ $mr->maintenance_date->format('F j, Y') }} · 
                    {{ $mr->asset ? $mr->asset->name : ($mr->workOrder->is_external_client ? 'Client: ' . $mr->workOrder->client_name : 'No Asset') }}
                </p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-5">
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
            @foreach([
                ['Asset/Location', $mr->asset ? $mr->asset->name : ($mr->workOrder->is_external_client ? 'Client: ' . $mr->workOrder->client_name : '—')],
                ['Technician', $mr->technician->name],
                ['Date', $mr->maintenance_date->format('M d, Y')],
                ['Work Order', $mr->workOrder ? $mr->workOrder->wo_number : '—'],
                ['Duration', $mr->duration_minutes . ' min'],
                ['Shutdown', $mr->shutdown_minutes . ' min']
            ] as [$l,$v])
