...
@section('breadcrumb')<span class="text-gray-400">/</span><a href="{{ route('maintenance-records.index') }}" class="hover:text-gray-800">Records</a><span class="text-gray-400">/</span><span class="text-gray-700 font-medium">Add Report</span>@endsection
@section('content')
<div class="max-w-none mx-auto pb-10">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900">Add Report</h1>
    </div>
    @if($workOrder)
        ...
        <form action="{{ route('maintenance-records.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div><label class="block text-sm font-medium text-gray-700 mb-1.5">Teknisi <span class="text-red-500">*</span></label>
                    <select name="technician_id" id="technicianSelect" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand">
                ...
                <div class="space-y-2">
                    <template x-for="(part, index) in parts" :key="index">
                        <div class="flex gap-2 items-center">
                            <select :name="'parts['+index+'][spare_part_id]'" x-model="part.spare_part_id" class="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand">
                                <option value="">Select part...</option>
                                @foreach($spareParts as $p)<option value="{{ $p->id }}">{{ $p->name }} ({{ $p->qty_actual }} {{ $p->unit }} available)</option>@endforeach
                            </select>
                            <input :name="'parts['+index+'][qty_used]'" x-model="part.qty_used" type="number" min="1" placeholder="Qty" class="w-24 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand">
            ...
            {{-- Photos --}}
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Photos (Multiple Selection Supported)</label>
