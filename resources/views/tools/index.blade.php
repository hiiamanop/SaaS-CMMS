...
<div class="space-y-5" x-data="{
    ...
    printCondition: '{{ request('condition', '') }}',
    triggerPrint() {
        let url = '{{ route('labels.print') }}?type=tool&format=' + this.printFormat;
        if (this.printCondition) {
            url += '&condition=' + encodeURIComponent(this.printCondition);
        }
        ...
        <div class="flex flex-col items-end gap-1" x-data="{ uploading: false }">
            <div class="flex items-center gap-2">
                ...
                <button type="button" @click="showPrintModal = true"
                        class="inline-flex items-center gap-2 px-3.5 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg text-sm font-bold hover:bg-gray-50 shadow-sm transition-all h-[38px]" title="Cetak Stiker Label QR Tools">
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 space-y-4 border border-gray-100"
            ...
            <div class="flex items-center justify-between pt-2 border-t border-gray-100">
                ...
                <div class="flex gap-2">
                    ...
                    <a :href="'/tools/' + selectedTool?.id" class="px-3.5 py-2 bg-brand text-gray-900 rounded-lg text-xs font-bold hover:bg-brand-600 transition-all flex items-center gap-1">Detail & Usage</a>
                    @if(auth()->user()->isAdminOrSupervisor())
                    <a :href="'/tools/' + selectedTool?.id + '/edit'" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg text-xs font-bold hover:bg-gray-100">Edit Tool</a>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 space-y-4 border border-gray-100"
            ...
            <div class="flex items-start justify-between border-b border-gray-100 pb-3">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-gray-900">Cetak Label QR Tools</h3>
                        <p class="text-xs text-gray-500">Stiker label peralatan kerja & instrumentasi</p>
                    </div>
                </div>
            ...
            <div class="space-y-4 text-sm">
                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">1. Filter Kondisi</label>
                    <select x-model="printCondition" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-xs font-semibold focus:ring-brand">
                        <option value="">Semua Kondisi ({{ $tools->total() }} Tools)</option>
                ...
                <div>
                    ...
                    <div class="space-y-2">
                        <label class="flex items-start gap-3 p-3 border rounded-xl cursor-pointer transition-all"
                            ...
                            <div>
                                <span class="font-bold text-gray-900 text-xs block">Stiker Roll 70 x 40 mm (Standar Kotak / Koper Alat)</span>
                                <span class="text-[11px] text-gray-500">1 stiker per halaman. Ukuran pas untuk koper alat & multimeter.</span>
                            </div>
                        </label>
                        <label class="flex items-start gap-3 p-3 border rounded-xl cursor-pointer transition-all"
                            ...
                            <div>
                                <span class="font-bold text-gray-900 text-xs block">Stiker Roll 50 x 30 mm (Label Mini Gagang Alat)</span>
                                <span class="text-[11px] text-gray-500">Cocok ditempel di gagang tang, obeng, dan kunci inggris.</span>
                            </div>
                        </label>
                        <label class="flex items-start gap-3 p-3 border rounded-xl cursor-pointer transition-all"
                            ...
                            <div>
                                ...
                                <span class="text-[11px] text-gray-500">Cetak banyak sekaligus menggunakan printer kantor biasa.</span>
                            </div>
                        </label>
                    </div>
                </div>
