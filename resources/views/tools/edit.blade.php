    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <form action="{{ route('tools.update', $tool) }}" method="POST" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div><label class="block text-sm font-medium text-gray-700 mb-1.5">Tool Code</label><input name="tool_code" value="{{ old('tool_code', $tool->tool_code) }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand @error('tool_code') border-red-400 @enderror">@error('tool_code')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror</div>
