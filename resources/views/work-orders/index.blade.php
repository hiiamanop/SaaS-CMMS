            ...
            <div class="flex gap-2">
                @if(!auth()->user()->isTechnician())
                    <a href="{{ route('work-orders.create') }}"
                        class="inline-flex items-center gap-2 px-4 py-2 bg-brand text-gray-900 rounded-lg text-sm font-medium hover:bg-brand-600 transition-all shadow-sm">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
            <form method="GET" class="flex flex-wrap gap-3">
                ...
                <select name="asset_id"
                    ...
                    @foreach($assets as $a)<option value="{{ $a->id }}" {{ request('asset_id') == $a->id ? 'selected' : '' }}>
                    {{ $a->name }}</option>@endforeach
                </select>
                ...
                <button type="submit"
                    class="px-4 py-2 bg-brand-dark text-white font-bold rounded-lg text-sm font-medium hover:bg-opacity-90 transition-all shadow-sm">Filter</button>
                <a href="{{ route('work-orders.index') }}"
                    class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg text-sm font-medium hover:bg-opacity-90 transition-all shadow-sm">Reset</a>
            </form>
        </div>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr
                                ...
                                <th class="px-5 py-4 text-left">WO #</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @foreach($workOrders as $wo)
                                        ...
                                        <tr class="hover:bg-opacity-90 transition-colors {{ $overdue ? 'bg-red-50/20' : '' }}">
                                            <td class="px-5 py-4 font-mono text-xs font-bold text-gray-700"><a
                                                    href="{{ route('work-orders.show', $wo) }}"
                                                    class="text-brand hover:underline">{{ $wo->wo_number }}</a></td>
                                            <td class="px-5 py-4">
                                            ...
                                            <td class="px-5 py-4 text-gray-600 text-xs">
                                                {{ $wo->assignees->isNotEmpty() ? $wo->assignees->pluck('name')->implode(', ') : '—' }}
                                            </td>
                                            <td class="px-5 py-4"><span
                                                    class="px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider {{ $pColors[$wo->priority] ?? '' }}">{{ $wo->priority }}</span>
                                            </td>
                                            <td class="px-5 py-4">
                                                ...
                                                @else
                                                                                <span class="px-2 py-0.5 rounded-md text-[10p
                                                     x                              ] font-bold uppercase tracking-wider {{ $sColors[$wo->status] ?? '' }}">
                                    ...
                                    <td class="p        x-5 py-4 text-xs {{ $overdue ? 'text-red-600 font-bold' : 'text-gray-500' }}">{{ $wo->due_date->format('d M Y') }}</td>
