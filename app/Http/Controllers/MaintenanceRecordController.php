...
class MaintenanceRecordController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'followup_due_date' => 'required_if:status_after,pending,failure|nullable|date',
        ...
        try {
            ...
            if (!empty($validated['work_order_id'])) {
                ...
                if ($wo) {
                    ...
                    // Auto-create Follow-Up WO if pending/failure
                    if (in_array($validated['status_after'], ['pending', 'failure'])) {
                        $newWo = WorkOrder::create([
                            'wo_number' => WorkOrder::generateNumber(),
                            'title' => '[FOLLOW-UP] ' . $wo->title,
                            ...
                            'is_external_client' => $wo->is_external_client,
                            'client_name' => $wo->client_name,
                            'description' => "Autogenerated follow-up from " . strtoupper($validated['status_after']) . " Record #" . $record->record_number . "\n\nOriginal Issue: " . $wo->description,
                            'status' => 'open'
                        ]);
                        
                        $newWo->assignees()->sync($wo->assignees->pluck('id')->toArray());
                        ...
                        \App\Models\WorkOrderActivityLog::create([
                            ...
                            'from_status' => null,
                            'to_status' => 'open',
                            'notes' => 'Auto-created follow-up work order',
                        ]);

                        if ($newWo->assignees->isNotEmpty()) {
                            foreach ($newWo->assignees as $assignee) {
                                \App\Models\Notification::create([
                                    'user_id' => $assignee->id,
                                    'type' => 'new_wo',
                                    'title' => 'Follow-Up Work Order Assigned',
                                    'message' => "You have been assigned Follow-Up Work Order {$newWo->wo_number}: {$newWo->title}",
                                    'url' => '/work-orders/' . $newWo->id,
                                ]);
                            }
                        }
                    }
    ...
    public function update(Request $request, MaintenanceRecord $maintenanceRecord)
    {
        ...
        DB::transaction(function() use ($validated, $request, $maintenanceRecord) {
